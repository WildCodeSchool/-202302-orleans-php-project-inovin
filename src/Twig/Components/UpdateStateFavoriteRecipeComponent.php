...
class UpdateStateFavoriteRecipeComponent extends AbstractController
{
    ...
    public function updateState(): void
    {
        ...
        if (!$user->isInFavoritesRecipes($this->recipe)) {
            $user->addFavoritesRecipe($this->recipe);
            $this->isLiked = true;
        } else {
            $user->removeFavoritesRecipe($this->recipe);
        }
    ...
    public function getState(): void
    {
        /** @var User $user */
        $user = $this->security->getUser();
        $this->isLiked =  $user->isInFavoritesRecipes($this->recipe);
    }
}
